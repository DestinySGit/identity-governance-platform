...
import {
  getApplicationOwnershipDetail,
  getIdentityOwners,
} from "@/app/actions/entitlements";
import { getProfile } from "@/lib/auth";
import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";
import { ApplicationOwnershipEditor } from "@/components/ownership/application-ownership-editor";
...
interface ApplicationOwnershipDetailPageProps {
  params: Promise<{ id: string }>;
}

export default async function ApplicationOwnershipDetailPage({
  params,
}: ApplicationOwnershipDetailPageProps) {
  const { id } = await params;
  const [detail, owners, profile] = await Promise.all([
    getApplicationOwnershipDetail(id),
    getIdentityOwners(),
    getProfile(),
  ]);
  ...
  const { application, assignedUsers, campaigns, reviewStatus } = detail;
      ...
      <div className="flex items-start justify-between">
        <div>
          <Button variant="ghost" size="sm" asChild className="mb-2">
            <Link href="/ownership/applications">← Back to applications</Link>
          </Button>
          ...
          <div className="flex gap-2 mt-3">
            {application.criticality_level && (
              <Badge
                variant={
                  criticalityVariant[application.criticality_level as CriticalityLevel]
                }
              >
                {application.criticality_level}
              </Badge>
        ...
        {isAdmin && (
          <ApplicationOwnershipEditor application={application} owners={owners} />
        )}
      </div>
        ...
        <div className="rounded-lg border p-6 space-y-3">
          ...
          {application.owner ? (
            <dl className="grid grid-cols-2 gap-2 text-sm">
              ...
              <dd>
                <Link
                  href={`/identities/${application.owner.id}`}
              ...
              <dd>{application.owner.email}</dd>
              <dt className="text-muted-foreground">Department</dt>
              <dd>{application.owner.department || "—"}</dd>
              <dt className="text-muted-foreground">Criticality</dt>
              <dd>{application.criticality_level || "—"}</dd>
              <dt className="text-muted-foreground">Review Frequency</dt>
              <dd>
                {application.review_frequency
                  ? frequencyLabels[application.review_frequency]
                  : "—"}
              </dd>
            </dl>
          ) : (
